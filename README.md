...
**The socket is open before the session behind it is, and frames sent in the gap are discarded.** A Client Portal Gateway completes the WebSocket upgrade locally, logs `WebSocket handshake complete`, and only afterwards connects upstream and logs `connected to server`. Anything written in between is dropped without a reply — no error, no close, nothing in the gateway's log where the frame should be. The subscription it carried then delivers nothing for the life of that connection, while the transport reports `Connected`, the subscribe call returns cleanly and heartbeats keep arriving. An A/B against a paper gateway settles it: subscribing immediately gave 0 messages in 30 seconds and no `smd+…` in the gateway log; the identical run delayed 3 seconds gave 4 frames and a full field set. So the transport writes nothing at all — `tic` included — until IBKR answers with the `system` message carrying `success`, on the first connection and on every reconnect, and an open whose confirmation does not arrive within `Streaming.SessionConfirmationTimeout` (10 seconds) fails rather than sending into the void.

**A historical bar's volume is not a number of shares.** `GET /iserver/marketdata/history` divides every bar's `v` by a `volumeFactor` it reports on the envelope, and that factor changes from response to response — a live gateway sent 40 where IBKR's own published example sends 100. AAPL's daily bar came back as `142573.625` on a session a snapshot put at 5,660,173 shares; times the factor it is 5,702,945, which is the same number to within the bar lagging the tape and counting regular hours only. `HistoricalBar.Volume` is the share count with the factor applied, `RawVolume` is the figure as sent, and `HistoricalBars.VolumeFactor` carries the divisor. The sibling `priceFactor` is not the same case: a bar's prices arrive as prices, and it scales only the envelope's `high` and `low` summary strings.
...
**The same question can arrive over the socket, unasked.** The `ntf` topic is documented as a brief notification — `id`, `title`, `text`, `url` — under a single `args` object. A live gateway sends `args` as an array, and one of the payloads recorded in it is not a notification at all but the limit-price constraint prompt: `orderId`, `reqId`, `messageId: "p12"`, three `options` and `prompt: true`, arriving unsolicited rather than as the answer to a submission. So `StreamingNotification.Args` is a list, and each element is either a `StreamingNotificationArgs.Notice` or a `.Prompt`. A prompt names its order by IBKR's own numeric identifier, which is what `DismissServerPromptAsync` takes; the notice alongside it was warning 118, dating a resting order's automatic cancellation.
...
**A watchlist's identifier is documented as a unique string of digits, and neither half holds.** `POST /iserver/watchlist` documents `id` as "Must be a number, digits 0-9 only. Must be unique relative to other watchlist IDs." IBKR gave some older lists negative identifiers — three of fifteen on one paper username, `-6062842538047152184` among them — and a live gateway takes a write under one exactly as it takes one under a positive id. And an identifier already in use is not refused: the list under it is replaced in place, with no duplicate and no warning. So `CreateAsync` accepts a leading minus and still refuses anything else before it is sent, since what IBKR does with a letter is undocumented; and because a create can overwrite, check `GetAllAsync` for the identifier first unless replacing the list is the point.
